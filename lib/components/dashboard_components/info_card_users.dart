...
class InfoCardUsers extends StatefulWidget {
  final List<Users> usersdata;

  const InfoCardUsers({Key? key, required this.usersdata}) : super(key: key);

  @override
  State<InfoCardUsers> createState() => _InfoCardUsers();
}

class _InfoCardUsers extends State<InfoCardUsers> {
  List<Users> newUsers = [];
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              bottom: 30,
              child: Text(
                widget.usersdata.length.toString(),
                style: TextStyle(
                  fontSize: 30,
                  fontWeight: FontWeight.w600,
                  color: kPrimary1,
  ...
  serilaizedData() {
    ...
    setState(() {
      newUsers = widget.usersdata
          .where(
              (element) => element.date!.toDate().isAfter(DateTime.now().subtract(Duration(days: 31))))
          .toList();
      //getting everyday count
      day1 = widget.usersdata.where((element) => element.date! == distinctDate[0]).toList();
      day2 = widget.usersdata.where((element) => element.date! == distinctDate[1]).toList();
      day3 = widget.usersdata.where((element) => element.date! == distinctDate[2]).toList();
      day4 = widget.usersdata.where((element) => element.date! == distinctDate[3]).toList();
      day5 = widget.usersdata.where((element) => element.date! == distinctDate[4]).toList();
      day6 = widget.usersdata.where((element) => element.date! == distinctDate[5]).toList();
      day7 = widget.usersdata.where((element) => element.date! == distinctDate[6]).toList();
